...
def modify_admin(
    ...
    if username != admin.username and dbadmin.is_sudo:
        raise HTTPException(
            status_code=403,
            detail="You're not allowed to edit another sudoers account. Use freeway-cli instead.",
        )
...
def remove_admin(username: str, db: DBDep, admin: SudoAdminDep):
    ...
    if dbadmin.is_sudo:
        raise HTTPException(
            status_code=403,
            detail="You're not allowed to delete sudoers accounts. Use freeway-cli instead.",
        )
